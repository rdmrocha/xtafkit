...
impl XexHeader {
    pub fn read<R: Read + Seek>(mut reader: R) -> Result<XexHeader> {
    ...
    fn check_magic_bytes<R: Read + Seek>(mut reader: R) -> Result<()> {
        let mut buf = [0_u8; 4];
        reader.read_exact(&mut buf).map_err(FatxError::Io)?;

        reader.seek(SeekFrom::Current(-4)).map_err(FatxError::Io)?;

        if buf != "XEX2".as_bytes() {
            return Err(FatxError::Other(
                "missing 'XEX2' magic bytes in XEX header".to_string(),
            ));
        }
    ...
    fn read_checked<R: Read + Seek>(mut reader: R) -> Result<XexHeader> {
        let header_offset = reader.stream_position().map_err(FatxError::Io)?;

        let _ = reader.read_u32::<BE>().map_err(FatxError::Io)?;

        let module_flags = reader.read_u32::<BE>().map_err(FatxError::Io)?;
        let module_flags = XexModuleFlags::from_bits_truncate(module_flags);

        let code_offset = reader.read_u32::<BE>().map_err(FatxError::Io)?;

        let _ = reader.read_u32::<BE>().map_err(FatxError::Io)?;

        let certificate_offset = reader.read_u32::<BE>().map_err(FatxError::Io)?;

        let mut fields: XexHeaderFields = Default::default();
        let field_count = reader.read_u32::<BE>().map_err(FatxError::Io)?;

        for _ in 0..field_count {
            let key = reader.read_u32::<BE>().map_err(FatxError::Io)?;
            let value = reader.read_u32::<BE>().map_err(FatxError::Io)?;

            let key = XexHeaderFieldId::try_from(key).ok();
            type Key = XexHeaderFieldId;

            if let Some(Key::ExecutionId) = key {
                let offset = reader.stream_position().map_err(FatxError::Io)?;
                reader
                    .seek(SeekFrom::Start(header_offset + (value as u64)))
